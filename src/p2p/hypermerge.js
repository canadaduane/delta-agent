...
class Hypermerge extends EventEmitter {
  ...
  joinSwarm(opts = {}) {
    this._ensureReady();
    log("joinSwarm", this.core.archiver.changes.discoveryKey);
    ...
    return () => {
      ...
      this._loadMetadata(actorId).then(() => {
        ...
        return this._loadBlocksWithDependencies(
          ...
          const doc = this.find(docId);
          const handles = this._handles(docId);
  ...
  _applyBlocks(docId, blocks) {
    ...
    this._applyChanges(
      docId,
      blocks.map(block => {
        log("_applyBlocks block", JSON.parse(block));
  ...
  _applyChanges(docId, changes) {
    ...
    if (changes.length > 0) {
      ...
      // log("_applyChanges changes", filteredChanges);
